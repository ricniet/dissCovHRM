...
covHRM_mod = function(){
  ...
  for (i in 1:NN) { 
    ...
    for (k in 1:K) {
      ...
      a[i,k] <- k - xi[subject[i],item[i]] - eta[cov1[i]]
      b[i,k] <- -pow(a[i,k],2)
...
covHRM_data <- list("subject","rater","x","item","cov1",
                    "NN","N","J","R","K","S",
                    "sd.psi","sd.eta","sd.beta","sd.kappa")
...
write.csv(covHRM_results, file='./justTests/test11/covHRMf_RHOonly_test13_N500R50J8K5_3ch16000i8000b16t.csv')
save(covHRM_jags, file='./justTests/test11/covHRMf_RHOonly_test13_N500R50J8K5_3ch16000i8000b16t.Rdat')

source('./justTests/assessTests.R')

assess_etaOnly(jagsObj = covHRM_jags, 
       R = R,
       filePath = './justTests/test13/',
       eval = c(1:47,49),
       random = F)
